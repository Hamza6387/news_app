...
class NewsService {
  ...
  static Future<List<Article>> fetchTopHeadlines() async {
    try {
      ...
      final url = '$_newsApiBaseUrl/top-headlines?country=us&apiKey=$_newsApiKey';
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode) {
          debugPrint('✅ Successfully fetched ${articles.length} articles from NewsAPI');
        }
  ...
  static Future<List<Article>> searchNews(String query) async {
    ...
    try {
      final url = '$_newsApiBaseUrl/everything?q=${Uri.encodeComponent(query)}&apiKey=$_newsApiKey&language=en&sortBy=publishedAt';
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] != 'ok') {
          throw Exception('NewsAPI Search Error: ${data['message'] ?? 'Unknown error'}');
        }
  ...
  static List<Article> _getMockArticles() {
    return [
      Article(
        ...
        description: 'OpenAI unveils its most advanced language model yet, featuring enhanced reasoning, creativity, and problem-solving abilities.',
        ...
        sourceName: 'Reuters',
        content: 'OpenAI continues to push the boundaries of artificial intelligence with significant new developments...',
      ),
      Article(
        title: 'Stock Markets Hit Record Highs Amid Tech Rally',
        description: 'Major stock indices reach all-time highs as technology companies lead the market surge.',
        ...
        content: 'Financial markets demonstrate strong performance with technology sector leading the charge...',
      ),
      Article(
        title: 'Climate Summit Reaches Historic Agreement on Renewable Energy',
        description: 'World leaders commit to ambitious new targets for clean energy transition and carbon reduction.',
        ...
        sourceName: 'BBC News',
        content: 'International cooperation on climate change reaches new milestones with comprehensive agreements...',
      ),
    ];
  }
} 